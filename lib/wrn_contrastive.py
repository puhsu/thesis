"Altered copy of wrn batch-norm -> layer norm, with projection head for contrastive learning"
...
class WideResNet(nn.Module):
    "WRN with nonlinear projection head and group normalization"
    def __init__(self, num_groups, N, k=1, drop_p=0.0, start_nf=16, inp_nf=3):
        super().__init__()
        n_channels = [start_nf]
        ...
        proj_shape = n_channels[-1]
        self.features = nn.Sequential(*layers)
        self.proj_head = nn.Sequential(nn.Linear(proj_shape, proj_shape), nn.ReLU(), nn.Linear(proj_shape, proj_shape))
        self.apply(weight_init)

    def forward(self, x):
        return self.proj_head(self.features(x))
